        function runPandoraApp() {
            ...
            const sindicatoSidebarWelcomeLogo = document.getElementById('sindicato-sidebar-welcome-logo');
            ...
            const ES_TERRITORY_TREE = (window.PANDORA_GEO && window.PANDORA_GEO.ES_TERRITORY_TREE) || [];
            const ES_MUNICIPALITY_ID_ALIASES = (window.PANDORA_GEO && window.PANDORA_GEO.ES_MUNICIPALITY_ID_ALIASES) || {};
            const IE_COUNTIES = (window.PANDORA_GEO && window.PANDORA_GEO.IE_COUNTIES) || [];

            function getSpainGeoTeamsFlat() {
                if (window.PANDORA_GEO && typeof window.PANDORA_GEO.getSpainGeoTeamsFlat === 'function') {
                    return window.PANDORA_GEO.getSpainGeoTeamsFlat();
                }
                return [];
            }

            const GEO_TEAMS_ES = (window.PANDORA_GEO && window.PANDORA_GEO.GEO_TEAMS_ES) || getSpainGeoTeamsFlat();

            function getCcaaTeamId(ccaaId) {
                if (window.PANDORA_GEO && window.PANDORA_GEO.getCcaaTeamId) {
                    return window.PANDORA_GEO.getCcaaTeamId(ccaaId);
                }
                return ccaaId ? `ccaa-${ccaaId}` : '';
            }

            function getProvinceTeamId(provinceId) {
                if (window.PANDORA_GEO && window.PANDORA_GEO.getProvinceTeamId) {
                    return window.PANDORA_GEO.getProvinceTeamId(provinceId);
                }
                return provinceId ? `province-${provinceId}` : '';
            }

            function getComarcaTeamId(comarcaId) {
                if (window.PANDORA_GEO && window.PANDORA_GEO.getComarcaTeamId) {
                    return window.PANDORA_GEO.getComarcaTeamId(comarcaId);
                }
            ...
            function getLocalePack() {
                return activeLocale === 'es' ? window.PANDORA_ES : window.PANDORA_IE;
            }

            function getLocaleUi(locale) {
                const loc = locale || activeLocale;
                const pack = loc === 'es' ? window.PANDORA_ES : window.PANDORA_IE;
            ...
            window.PANDORA_DEMO_PERSON_NAMES = {
            ...
            function demoName(personId) {
                ...
                const row = window.PANDORA_DEMO_PERSON_NAMES[personId];
                if (!row) return '';
            ...
            function formatMilitantContactRow(contact) {
                ...
                if (activeLocale === 'es' && window.PANDORA_ES && window.PANDORA_ES.localizeMilitantContact) {
                    return window.PANDORA_ES.localizeMilitantContact(base);
                }
                return base;
            }
            ...
            function buildPartyStructureHtml(opts = {}) {
                if (activeLocale === 'es' && window.PANDORA_ES && window.PANDORA_ES.buildPartyStructureHtml) {
                    return window.PANDORA_ES.buildPartyStructureHtml(opts);
                }
                if (activeLocale === 'ie' && window.PANDORA_IE && window.PANDORA_IE.buildPartyStructureHtml) {
                    return window.PANDORA_IE.buildPartyStructureHtml(opts);
                }
            ...
            function buildPartyCandidacyHtml(opts = {}) {
                if (activeLocale === 'es' && window.PANDORA_ES && window.PANDORA_ES.buildPartyCandidacyHtml) {
                    return window.PANDORA_ES.buildPartyCandidacyHtml(opts);
                }
                if (activeLocale === 'ie' && window.PANDORA_IE && window.PANDORA_IE.buildPartyCandidacyHtml) {
                    return window.PANDORA_IE.buildPartyCandidacyHtml(opts);
                }
                const ui = getLocaleUi();
            ...
            const LOCALE_STORAGE_KEY = 'pandora-locale';
            const LOCALE_CONFIG = {
                ie: {
            ...
            function applyLocaleToDocument() {
                ...
                document.body.classList.add(activeLocale === 'es' ? 'pandora-locale-es' : 'pandora-locale-ie');
            ...
            function initLocaleEarly() {
                activeLocale = readStoredLocale() || window.__pandoraEarlyLocale || 'es';
                syncEuropePreCheckedOverride(activeLocale);
                refreshGeoTeamsForLocale();
            ...
            function setActiveLocale(locale, options = {}) {
                ...
                document.body.classList.remove('pandora-booting');
            }

            window.__pandoraApplyLocale = function (locale) {
                setActiveLocale(locale);
            };
            if (window.__pandoraLocaleQueue) {
                setActiveLocale(window.__pandoraLocaleQueue);
            ...
            function refreshLocaleNavLabels() {
                const ui = getLocaleUi();
                const welcome = document.querySelector('.template-welcome');
                if (welcome && ui.welcome) welcome.textContent = ui.welcome;
                const mapPlaceholder = document.querySelector('.map-placeholder');
                if (mapPlaceholder && ui.mapHint) mapPlaceholder.innerHTML = ui.mapHint;
                if (mobileToggle) {
                    mobileToggle.textContent = ui.sidebar || 'Sidebar';
                    mobileToggle.setAttribute('aria-label', ui.mobileToggleAria || ui.sidebar || 'Sidebar');
                }
            ...
            function relocalizeForLocale() {
                ...
                if (activeModule === SINDICATO_MODULE) {
                    if (activeSindicatoWorkplace && window.PANDORA_SINDICATO
                        && !window.PANDORA_SINDICATO.findWorkplace(activeLocale, activeSindicatoWorkplace)) {
            ...
            function buildCollectiveRepresentationHtml() {
                ...
                if (activeLocale === 'es' && window.PANDORA_ES && window.PANDORA_ES.buildCollectiveRepresentationHtml) {
                    return window.PANDORA_ES.buildCollectiveRepresentationHtml(COLLECTIVE_REPRESENTATION, formatPct, buildCollectiveRepRowHtml);
                }
            ...
            function buildCollectivesGeneralCoordinationHtml() {
                if (activeLocale === 'es' && window.PANDORA_ES && window.PANDORA_ES.buildCollectivesGeneralCoordinationHtml) {
                    return window.PANDORA_ES.buildCollectivesGeneralCoordinationHtml();
                }
                const directorate = [
            ...
            function getSindicatoCopy() {
                const pack = window.PANDORA_SINDICATO;
            ...
            function getUserWorkplaceId() {
                ...
                if (window.PANDORA_SINDICATO && window.PANDORA_SINDICATO.findWorkplace(activeLocale, preferred)) {
                    return preferred;
                }
                const list = window.PANDORA_SINDICATO?.getWorkplaces(activeLocale) || [];
                return list[0]?.id || '';
            }
            ...
            function refreshSelfNavLabels() {
                ...
                const wp = window.PANDORA_SINDICATO?.findWorkplace(activeLocale, wpId);
            ...
            function syncSelfSindicatoWorkspace() {
                if (!mapTextDisplay || !window.PANDORA_SINDICATO) return;
                const wpId = getUserWorkplaceId();
                mapTextDisplay.innerHTML = window.PANDORA_SINDICATO.buildWorkspaceHtml(
                    activeLocale,
                    'workplaces',
                    wpId,
            ...
            function syncSelfSindicatoLocationBridge() {
                ...
                const wp = window.PANDORA_SINDICATO.findWorkplace(activeLocale, wpId);
                if (!wp) return;
                mapTextDisplay.innerHTML = window.PANDORA_SINDICATO.buildLocationBridgeHtml(activeLocale, wp);
            ...
            function setSelfSub(subId) {
                activeSelfSub = subId === 'sindicato' ? 'sindicato' : 'partido';
                ...
                if (activeSelfSub === 'sindicato') {
                    ...
                    setSelfPartySection(activeSelfPartySection || 'public-profile');
                }
            }

            function rebuildSindicatoWorkplaceSelect() {
                if (!sindicatoWorkplaceSelect || !window.PANDORA_SINDICATO) return;
                const c = getSindicatoCopy();
                const q = sindicatoWorkplaceFilter.trim().toLowerCase();
                const list = window.PANDORA_SINDICATO.getWorkplaces(activeLocale).filter((w) =>
            ...
            function rebuildSindicatoViviendaSelects() {
                if (!window.PANDORA_SINDICATO || !sindicatoViviendaParentSelect || !sindicatoViviendaTerritorySelect) return;
                const c = getSindicatoCopy();
                const parents = window.PANDORA_SINDICATO.getTerritoryParents(activeLocale);
                ...
                const subs = activeSindicatoViviendaParent
                    ? window.PANDORA_SINDICATO.getSubterritoriesForParent(activeLocale, activeSindicatoViviendaParent)
                    : [];
            ...
            function setSindicatoViviendaParent(parentId) {
                if (!window.PANDORA_SINDICATO) return;
                const parents = window.PANDORA_SINDICATO.getTerritoryParents(activeLocale);
            ...
            function renderSindicatoMapTerritoryTree() {
                if (!sindicatoMapTerritoryTreeMount || !window.PANDORA_SINDICATO?.buildTerritoryScopeTreeHtml) return;
                sindicatoMapTerritoryTreeMount.innerHTML = window.PANDORA_SINDICATO.buildTerritoryScopeTreeHtml(
                    activeLocale,
            ...
            function setSindicatoMapTerritory(territoryId) {
                if (!window.PANDORA_SINDICATO) return;
                const allSubs = window.PANDORA_SINDICATO.getSubterritories(activeLocale);
            ...
            function renderSindicatoSectoresTree() {
                if (!sindicatoSectoresTreeMount || !window.PANDORA_SINDICATO?.buildSectoresSidebarTreeHtml) return;
                sindicatoSectoresTreeMount.innerHTML = window.PANDORA_SINDICATO.buildSectoresSidebarTreeHtml(
                    activeLocale,
            ...
            function renderSindicatoAddCompanySidebarForm() {
                if (!sindicatoAddCompanySidebarMount || !window.PANDORA_SINDICATO?.buildAddCompanyFormHtml) return;
                sindicatoAddCompanySidebarMount.innerHTML = window.PANDORA_SINDICATO.buildAddCompanyFormHtml(activeLocale);
            }

            function getSindicatoFeedScopeWorkplaces() {
                if (!window.PANDORA_SINDICATO) return [];
                if (activeSindicatoFeedScope === 'sectores' && activeSindicatoFeedSectorId) {
                    const node = window.PANDORA_SINDICATO.resolveSectorNode(activeLocale, activeSindicatoFeedSectorId);
                    return node?.workplaces || [];
                }
                if (activeSindicatoFeedScope === 'territorios' && activeSindicatoFeedTerritoryId) {
                    const terr = window.PANDORA_SINDICATO.getSubterritoryById(activeLocale, activeSindicatoFeedTerritoryId);
            ...
            function rebuildSindicatoFeedCompanySelect() {
                if (!sindicatoFeedCompanySelect || !window.PANDORA_SINDICATO) return;
                const c = getSindicatoCopy();
            ...
            function renderSindicatoFeedSidebar() {
                ...
                if (sindicatoFeedScopeTreeMount) {
                    ...
                    if (showTree && window.PANDORA_SINDICATO?.buildFeedScopeTreeHtml) {
                        const activeId = activeSindicatoFeedScope === 'territorios'
                            ? activeSindicatoFeedTerritoryId
                            : activeSindicatoFeedSectorId;
                        sindicatoFeedScopeTreeMount.innerHTML = window.PANDORA_SINDICATO.buildFeedScopeTreeHtml(
                            activeLocale,
            ...
            function buildSindicatoWorkspaceHtml() {
                if (!window.PANDORA_SINDICATO) {
                    const errCopy = window.PANDORA_SINDICATO?.COPY?.[activeLocale === 'es' ? 'es' : 'ie']?.moduleLoadError;
                    return `<div class="sindicato-panel"><p class="template-muted">${errCopy || 'Syndicate module failed to load.'}</p></div>`;
                }
                return window.PANDORA_SINDICATO.buildWorkspaceHtml(
                    activeLocale,
                    activeSindicatoSub,
                    activeSindicatoWorkplace,
            ...
            function applySindicatoHashRoute() {
                const route = window.PANDORA_SINDICATO?.parseSindicatoRoute(location.hash);
                if (!route) {
            ...
            function syncSindicatoLocationBridge() {
                if (!mapTextDisplay || !window.PANDORA_SINDICATO || !activeSindicatoWorkplace) return;
                const wp = window.PANDORA_SINDICATO.findWorkplace(activeLocale, activeSindicatoWorkplace);
                if (!wp) return;
                mapTextDisplay.innerHTML = window.PANDORA_SINDICATO.buildLocationBridgeHtml(activeLocale, wp);
            }
            ...
            function isSindicatoTextWorkspace() {
                if (activeModule !== SINDICATO_MODULE) return false;
                if (!activeSindicatoSub) return true;
            ...
            function ensureSindicatoDefaultBoundaryLayer() {
                const layer = window.PANDORA_SINDICATO?.getDefaultBoundaryLayerForLocale(activeLocale)
            ...
            function highlightSindicatoTerritoryBoundaries(territoryId) {
                const link = window.PANDORA_SINDICATO?.getTerritoryBoundaryLink(activeLocale, territoryId);
            ...
            function syncSindicatoMapMarkers() {
                if (!currentMap || !window.PANDORA_SINDICATO || typeof L === 'undefined') return;
                clearSindicatoMapMarkers();
                if (!isSindicatoMapVisible()) return;
                sindicatoMarkersLayer = L.layerGroup();
                window.PANDORA_SINDICATO.getWorkplaces(activeLocale).forEach((wp) => {
            ...
            function focusSindicatoWorkplaceOnMap(workplaceId) {
                if (!currentMap || !window.PANDORA_SINDICATO || !workplaceId) return;
                const wp = window.PANDORA_SINDICATO.findWorkplace(activeLocale, workplaceId);
            ...
            function rebuildSindicatoUnionSelect() {
                if (!sindicatoUnionSelect || !window.PANDORA_SINDICATO) return;
                const c = window.PANDORA_SINDICATO.t(activeLocale);
                const placeholder = c.selectUnion || 'Select union…';
                const unions = window.PANDORA_SINDICATO.getUnions(activeLocale);
            ...
            function handleSindicatoWorkspaceClick(e) {
                ...
                if (gotoVivienda) {
                    ...
                    if (!terrId || !window.PANDORA_SINDICATO) return;
                    const terr = window.PANDORA_SINDICATO.getSubterritoryById(activeLocale, terrId);
                    if (activeModule !== SINDICATO_MODULE) setActiveModule(SINDICATO_MODULE);
                ...
                if (strikePoll && (e.type === 'submit' || e.target.type === 'submit') && window.PANDORA_SINDICATO) {
                    e.preventDefault();
                    ...
                    if (wpId && vote) {
                        window.PANDORA_SINDICATO.castStrikeVote(wpId, vote);
                        syncSindicatoWorkspace();
                    }
                    return;
                }
                ...
                if (moderateBtn && window.PANDORA_SINDICATO) {
                    ...
                    if (reportId && action) {
                        window.PANDORA_SINDICATO.moderateReport(reportId, action);
                        syncSindicatoWorkspace();
                    }
                    return;
                }
                ...
                if (reportForm && (e.type === 'submit' || e.target.type === 'submit') && window.PANDORA_SINDICATO) {
                    e.preventDefault();
                    ...
                    if (wpId && type) {
                        window.PANDORA_SINDICATO.submitReport(activeLocale, wpId, type, detail);
                        reportForm.reset();
                        syncSindicatoWorkspace();
                    }
                    return;
            ...
            function updateModuleNavTrees() {
                ...
                if (selfSindicatoBlock) {
                    selfSindicatoBlock.hidden = !onSelf || activeSelfSub !== 'sindicato';
                }
                if (selfPartyBlock) {
                    selfPartyBlock.hidden = !onSelf || activeSelfSub !== 'partido';
                }
            ...
            function navigateSindicatoMapTerritoryFromSelection(selection) {
                ...
                if (activeModule !== SINDICATO_MODULE) setActiveModule(SINDICATO_MODULE);
                if (activeSindicatoSub !== 'map') setSindicatoSub('map');
                const terrId = window.PANDORA_SINDICATO?.resolveSindicatoTerritoryFromBoundary(
                    activeLocale,
                    selection.type,
                    selection.name
                );
            ...
            function getPlatformRankLadderArticle() {
                if (window.PANDORA_IE && window.PANDORA_IE.PLATFORM_RANK_LADDER) {
                    return window.PANDORA_IE.PLATFORM_RANK_LADDER;
                }
                if (activeLocale === 'es' && window.PANDORA_ES && window.PANDORA_ES.PLATFORM_RANK_LADDER) {
                    return window.PANDORA_ES.PLATFORM_RANK_LADDER;
                }
            ...
            function buildPartyWikiIndexHtml(locale) {
                ...
                if (loc === 'es' && window.PANDORA_ES && window.PANDORA_ES.buildPartyWikiIndexHtml) {
                    return window.PANDORA_ES.buildPartyWikiIndexHtml();
                }
                if (window.PANDORA_IE && window.PANDORA_IE.buildPartyWikiIndexHtml) {
                    return window.PANDORA_IE.buildPartyWikiIndexHtml();
                }
            ...
            function buildSelfAccountDetailsSeed(locale) {
                const loc = locale === 'es' ? 'es' : 'ie';
                if (loc === 'es' && window.PANDORA_ES && window.PANDORA_ES.buildSelfAccountDetailsSeed) {
                    return window.PANDORA_ES.buildSelfAccountDetailsSeed();
                }
                return `<!-- locale:ie -->
            ...
            function getWikiArticle(slug) {
                if (activeLocale === 'es' && window.PANDORA_ES && window.PANDORA_ES.WIKI_ARTICLES && window.PANDORA_ES.WIKI_ARTICLES[slug]) {
                    return window.PANDORA_ES.WIKI_ARTICLES[slug];
                }
                if (activeLocale === 'ie' && window.PANDORA_IE && window.PANDORA_IE.WIKI_ARTICLES && window.PANDORA_IE.WIKI_ARTICLES[slug]) {
                    return window.PANDORA_IE.WIKI_ARTICLES[slug];
                }
            ...
            function getTalkNewsCatalog() {
                ...
                if (window.PANDORA_IE && window.PANDORA_IE.TALK_NEWS_ARTICLES) return window.PANDORA_IE.TALK_NEWS_ARTICLES;
                return {};
            }
            ...
            function getTalkForumCatalog() {
                ...
                if (window.PANDORA_IE && window.PANDORA_IE.TALK_FORUM_THREADS) return window.PANDORA_IE.TALK_FORUM_THREADS;
                return {};
            }
            ...
            function getVoteCastUi() {
                if (activeLocale === 'es' && window.PANDORA_ES && window.PANDORA_ES.voteCastUi) {
                    return window.PANDORA_ES.voteCastUi;
                }
            ...
            function getCuentaPersonalSeedBundle() {
                if (activeLocale === 'es' && window.PANDORA_ES && window.PANDORA_ES.cuentaPersonalSeed) {
                    return window.PANDORA_ES.cuentaPersonalSeed;
                }
            ...
            function getMessageThreads() {
                ...
                const base = (loc === 'es' && window.PANDORA_ES && window.PANDORA_ES.messageThreads)
                    ? window.PANDORA_ES.messageThreads
                    : MESSAGE_THREADS_IE;
            ...
            function buildSelfAdminSubmoduleSeeds() {
                ...
                const sindHtml = window.PANDORA_SINDICATO?.buildCoordinationAdminHtml
                    ? window.PANDORA_SINDICATO.buildCoordinationAdminHtml(activeLocale)
            ...
            const VOTE_CAST_STORAGE_KEY = 'pandora:self-vote-cast';
            ...
            try {
                ...
                console.error('Pandora init error (partial UI may still work):', bootErr);
                showPandoraBootError(getPandoraMissingDependencies().concat(['init: ' + (bootErr && bootErr.message ? bootErr.message : String(bootErr))]));
            }
            ...
            function setSidebarOpen(isOpen) {
                ...
                mobileToggle.textContent = open ? 'Background' : 'Sidebar';
                mobileToggle.setAttribute('aria-label', open ? 'Show background' : 'Show sidebar');
            ...
            function getPandoraMissingDependencies() {
                const missing = [];
                if (typeof window.setupBoundaryControls !== 'function') {
                    missing.push('cartagrama-territories-bundle.js');
                }
                if (!window.PANDORA_GEO) {
                    missing.push('pandora-locale-geo-data.js');
                }
                if (!window.PANDORA_IE || !window.PANDORA_IE.localeUi) {
                    missing.push('pandora-locale-ie-content.js');
                }
                if (!window.PANDORA_IE || !window.PANDORA_IE.PARTY_TEAM_SUBMODULES) {
                    missing.push('pandora-locale-ie-party.js');
                }
                if (!window.PANDORA_ES) {
                    missing.push('pandora-locale-es-content.js');
                }
                return missing;
            }

            function showPandoraBootError(missing) {
                if (!missing.length || document.getElementById('pandora-boot-error')) return;
                const fromDrive = /drive\.google\.com|docs\.google\.com|googleusercontent\.com/i.test(location.hostname);
                const el = document.createElement('div');
                el.id = 'pandora-boot-error';
                el.className = 'pandora-boot-error';
                el.setAttribute('role', 'alert');
                el.innerHTML = `<strong>Pandora failed to start</strong>
                    <p>These embedded modules did not initialize:</p>
                    <ul>${missing.map((f) => `<li><code>${f}</code></li>`).join('')}</ul>
                    ${fromDrive
                        ? '<p><strong>Google Drive preview blocked script execution.</strong> Download <code>Pandora.html</code> and open it locally (Chrome → File → Open), not inside Drive’s preview.</p>'
                        : '<p>Try reloading the page. If you edited locale data, run <code>node scripts/bundle-pandora.mjs</code> to rebuild the single-file bundle. Map tiles still need internet (Leaflet loads from CDN).</p>'}`;
                const host = document.getElementById('console-content') || document.body;
                host.insertBefore(el, host.firstChild);
            }

            function assertPandoraDependencies() {
                const missing = getPandoraMissingDependencies();
                if (missing.length) showPandoraBootError(missing);
                return !missing.length;
            }

            function initBoundaryControlsOnce() {
                if (boundaryControlsInitialized) return;
                if (!assertPandoraDependencies()) return;
                syncEuropePreCheckedOverride(activeLocale);
                if (typeof window.setupBoundaryControls === 'function') {
            ...
            function syncMapWorkspacePlaceholder() {
                ...
                const ui = getLocaleUi();
                if (!activeModule) {
                    setMapPlaceholder(ui.welcome || 'Welcome to Pandora');
                } else if (isGeoTeamsMapWorkspace() || isSindicatoMapVisible()) {
            ...
            function updateSindicatoSidebarWelcomeLogo() {
                if (!sindicatoSidebarWelcomeLogo) return;
                const show = activeModule === SINDICATO_MODULE && !activeSindicatoSub;
                const logoSrc = window.PANDORA_SINDICATO?.LOGO_SRC;
                if (show && logoSrc) {
                    sindicatoSidebarWelcomeLogo.src = logoSrc;
                    sindicatoSidebarWelcomeLogo.hidden = false;
                } else {
                    sindicatoSidebarWelcomeLogo.hidden = true;
                }
            }
            ...
            function updateModuleBodyLabel() {
                ...
                if (!activeModule) {
                    ...
                    updateSindicatoSidebarWelcomeLogo();
                    return;
                }
                ...
                if (isLocalModuleActive()) {
                    ...
                    if (!activeSindicatoSub) {
                        title = c.welcomeTitle || labels.sindicato || 'SindicApp';
                    } else {
                        title = subs[activeSindicatoSub] || labels.sindicato || 'Syndicate';
                    }
                    if (activeSindicatoSub === 'workplaces' && activeSindicatoWorkplace && window.PANDORA_SINDICATO) {
                        const wp = window.PANDORA_SINDICATO.findWorkplace(activeLocale, activeSindicatoWorkplace);
                        const sections = c.sections || {};
                        const sec = sections[activeSindicatoSection] || activeSindicatoSection;
                        if (wp) title = `${wp.name} — ${sec}`;
                    } else if (activeSindicatoSub === 'sectores' && activeSindicatoSector && window.PANDORA_SINDICATO?.resolveSectorNode) {
                        const sectorNode = window.PANDORA_SINDICATO.resolveSectorNode(activeLocale, activeSindicatoSector);
                    ...
                    } else if (activeSindicatoSub === 'vivienda' && activeSindicatoViviendaTerritory && window.PANDORA_SINDICATO) {
                        const terr = window.PANDORA_SINDICATO.getSubterritoryById(activeLocale, activeSindicatoViviendaTerritory);
                        if (terr) title = `${subs.vivienda || 'Vivienda'} — ${terr.name}`;
                    } else if (activeSindicatoSub === 'unions' && activeSindicatoUnion && window.PANDORA_SINDICATO) {
                        const union = window.PANDORA_SINDICATO.findUnion(activeLocale, activeSindicatoUnion);
                    ...
                    if (activeSelfSub === 'sindicato') {
                        const c = getSindicatoCopy();
                        const wp = window.PANDORA_SINDICATO?.findWorkplace(activeLocale, getUserWorkplaceId());
            ...
            function syncPartyCandidacyWorkspace() {
                ...
                let html = '';
                if (activeLocale === 'es' && window.PANDORA_ES && typeof window.PANDORA_ES.buildPartyCandidacyHtml === 'function') {
                    html = window.PANDORA_ES.buildPartyCandidacyHtml(opts);
                } else if (window.PANDORA_IE && typeof window.PANDORA_IE.buildPartyCandidacyHtml === 'function') {
                    html = window.PANDORA_IE.buildPartyCandidacyHtml(opts);
                } else {
            ...
            if (territoryInfoBoxEl) {
                ...
                if (territoryInfoBoxCloseBtn && !territoryInfoBoxCloseBtn.dataset.pandoraCloseWired) {
                    territoryInfoBoxCloseBtn.dataset.pandoraCloseWired = '1';
            ...
            document.addEventListener('pandora-territory-selected', (e) => {
            ...
            document.addEventListener('pandora-territory-cleared', () => {
            ...
            if (templateModulePicker) {
                templateModulePicker.addEventListener('click', (e) => {
                    ...
                    if (org === 'colectivos') {
                        setActiveModule(COLLECTIVES_MODULE);
                        return;
                    }
                    if (org) {
                        if (activeModule !== PARTY_MODULE) setActiveModule(PARTY_MODULE);
            ...
            if (sindicatoNavTree) {
                sindicatoNavTree.addEventListener('submit', (e) => {
                    const form = e.target;
                    if (!form.matches?.('[data-sindicato-add-company]') || !window.PANDORA_SINDICATO) return;
                    e.preventDefault();
                    const fd = new FormData(form);
                    const company = window.PANDORA_SINDICATO.addCompany(activeLocale, {
                        name: fd.get('name'),
            ...
            function setActiveModule(moduleId) {
                ...
                if (activeModule === COLLECTIVES_MODULE) {
                    ...
                    refreshSelfNavLabels();
                    setSelfSub(activeSelfSub || 'partido');
                    updateModuleBodyLabel();
                } else if (activeModule === SINDICATO_MODULE) {
                    refreshSindicatoSidebarLabels();
                    setSindicatoSub('');
            ...
            function finishPandoraBoot() {
                try {
                    const bootLocale = readStoredLocale() || window.__pandoraEarlyLocale || 'es';
                    ...
                    console.error('Pandora startup error:', bootErr);
                    showPandoraBootError(getPandoraMissingDependencies().concat(['startup: ' + (bootErr && bootErr.message ? bootErr.message : String(bootErr))]));
                }
                document.body.classList.remove('pandora-booting');
                setSidebarOpen(true);
                assertPandoraDependencies();
            }

            if (typeof requestAnimationFrame === 'function') {
                requestAnimationFrame(function () {
                    setTimeout(finishPandoraBoot, 0);
                });
            } else {
                setTimeout(finishPandoraBoot, 0);
            }
        ...
        if (typeof requestAnimationFrame === 'function') {
            requestAnimationFrame(function () { setTimeout(runPandoraApp, 0); });
        } else {
            setTimeout(runPandoraApp, 0);
        }
